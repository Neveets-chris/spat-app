...
// ── Modal Formulaire ──────────────────────────────────────────────────────────
function ModalForm({ attribution, onClose, onSave }) {
  ...
  const ajouterOccupant = () => {
    if (!occupant.trim()) return;
    setForm({ ...form, occupants: [...form.occupants, occupant.trim()] });
    setOccupant("");
  };

  const supprimerOccupant = (nom) => {
    setForm({ ...form, occupants: form.occupants.filter((o) => o !== nom) });
  };
        ...
        <div className="flex justify-between items-center mb-5">
          <div>
            <h2 className="text-lg font-black text-[#0F2D56] dark:text-white">
              {attribution ? "Modifier l'attribution" : "Nouvelle attribution"}
            </h2>
          </div>
          <button
            onClick={onClose}
          ...
          <div>
            ...
            <div className="flex gap-2 mt-1">
              <input
                ...
                onKeyDown={(e) => e.key === "Enter" && ajouterOccupant()}
              ...
              />
              <button
                onClick={ajouterOccupant}
                className="px-3 py-2 bg-[#0F2D56] text-white rounded-lg text-sm hover:bg-[#1a3f75] transition font-bold"
              >
                +
              </button>
            </div>
            {form.occupants.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-2">
                {form.occupants.map((o) => (
                  <span
                    key={o}
                    className="flex items-center gap-1 px-2 py-1 bg-[#0F2D56]/10 dark:bg-gray-700 text-[#0F2D56] dark:text-gray-300 rounded-lg text-xs font-semibold"
                  >
                    {o}
                    <button
                      onClick={() => supprimerOccupant(o)}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-[#F7F5F0] dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-sm p-6 border border-[#E0DDD7] dark:border-gray-700">
        <div className="text-center mb-5">
          <div className="w-14 h-14 bg-amber-100 dark:bg-amber-900/30 rounded-full flex items-center justify-center text-2xl mx-auto mb-3">
            ⚠
          </div>
          <h2 className="text-lg font-black text-[#0F2D56] dark:text-white">
          ...
          <p className="text-xs text-gray-400 mt-2">
            Le logement sera remis en statut{" "}
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            className="flex-1 py-2 bg-amber-500 text-white rounded-xl font-semibold hover:bg-amber-600 transition text-sm"
          >
            Confirmer
          </button>
        </div>
...
export default function Attributions() {
  const [data, setData] = useState(ATTRIBUTIONS);
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Modals */}
      {detail && (
        <ModalDetail attribution={detail} onClose={() => setDetail(null)} />
      )}
      {finData && (
        <ModalFin
          attribution={finData}
      ...
      <p className="text-xs text-gray-400">
        {filtered.length} attribution{filtered.length > 1 ? "s" : ""}
      </p>
              ...
              >
                    ...
                    {a.occupants.length > 0 ? (
                      <>
                        {a.occupants.slice(0, 2).map((o) => (
                          <span
                            key={o}
                            className="text-xs bg-[#0F2D56]/10 dark:bg-gray-700 text-[#0F2D56] dark:text-gray-300 px-1.5 py-0.5 rounded-full"
                          >
                            {o.split(" ")[0]}
                          </span>
                        ))}
                ...
                <td className="px-5 py-3">
                  <div className="flex items-center gap-1.5">
                    ...
                    {a.statut === "Occupé" && (
                      <button
                        onClick={() => setFinData(a)}
                        className="group flex items-center justify-center w-8 h-8 rounded-lg bg-amber-100 dark:bg-amber-900/30 hover:bg-amber-500 transition-all duration-200 hover:scale-110"
                        title="Mettre fin à l'attribution"
                      >
                        <svg
                          className="w-4 h-4 text-amber-500 group-hover:text-white transition-colors"
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                          strokeWidth={2}
                    ...
                    <BoutonSupprimer onClick={() => handleDelete(a.id)} />
                  </div>
                </td>
              </tr>
            ))}
