    ...
    <div className="relative flex flex-col items-center">
      <div className="relative" style={{ width: size, height: size }}>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className={`text-xl font-black drop-shadow-lg ${theme.text}`}>
            {percentage}%
          </span>
        </div>
      </div>
      <p
        className={`mt-1 text-xs font-semibold uppercase tracking-wider ${theme.textSubtle}`}
      >
        {label}
      </p>
    </div>
...
function AttributionCard3D({
  ...
  const statutConfig = {
    ...
    Terminé: {
      bg: isDark
        ? "from-gray-500/10 to-gray-600/5 border-gray-500/30"
        ...
        <div className="space-y-3">
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              {attribution._reparationTerminee && (
                <span
              ...
              {joursRestants !== null &&
                ...
                attribution.statut === "Occupé" && (
                  <span
                    className={`text-xs ${joursRestants <= 7 ? "text-rose-400 animate-pulse" : "text-amber-400"}`}
                  >
          ...
          {/* Logement */}
          <div
            className={`flex items-center gap-3 p-3 rounded-xl ${isDark ? "bg-white/5" : "bg-gray-100"}`}
          >
          ...
          <div>
            <p className={`text-xs ${theme.textSubtle} mb-2`}>
              Occupants ({attribution.occupants.length})
            </p>
              ...
              {attribution.occupants.length > 0 ? (
                <>
                  {attribution.occupants.slice(0, 3).map((o, i) => (
                    <span
                      key={i}
                      className={`px-2 py-1 rounded-lg text-xs font-medium ${isDark ? "bg-white/10 text-white/80" : "bg-gray-200 text-gray-700"}`}
                    >
                      {o.split(" ")[0]}
                    </span>
                  ))}
                  {attribution.occupants.length > 3 && (
                    <span
                      className={`px-2 py-1 rounded-lg text-xs font-medium ${isDark ? "bg-white/5 text-white/60" : "bg-gray-100 text-gray-500"}`}
                    >
                      +{attribution.occupants.length - 3}
                    </span>
                  )}
  ...
  const data = useMemo(() => {
    ...
    attributions.forEach((a) => {
      ...
      const dateFin = a.date_fin
        ? new Date(a.date_fin.split("/").reverse().join("-"))
        : null;
      ...
      moisLabels.forEach((mois) => {
        ...
        if (monthIndex !== -1) {
          ...
          if (
            dateDebut.getMonth() === moisDate.getMonth() &&
            dateDebut.getFullYear() === moisDate.getFullYear()
          ) {
            parMois[mois].entrants++;
          }
          if (
            dateFin &&
            dateFin.getMonth() === moisDate.getMonth() &&
            dateFin.getFullYear() === moisDate.getFullYear()
          ) {
    ...
    >
      ...
      <div className="relative z-10">
        ...
        <div className="space-y-3">
              ...
              <div key={dept} className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between text-xs mb-1">
                    <span className={`font-medium truncate ${theme.text}`}>
                      {dept}
                    </span>
          ...
          {data.length === 0 && (
            <p className={`text-sm text-center py-4 ${theme.textSubtle}`}>
...
function ModalDetail3D({ attribution, onClose, isDark }) {
  ...
  return (
    ...
    >
      <div
        className={`rounded-t-3xl sm:rounded-3xl shadow-2xl w-full sm:max-w-md border overflow-hidden animate-in zoom-in-95 duration-300  max-h-[90vh] overflow-y-auto ${isDark ? "bg-gradient-to-br from-gray-900 to-gray-800 border-white/10" : "bg-white border-gray-200"}`}
      >
        <div
          className={`relative px-6 py-6 border-b ${isDark ? "bg-gradient-to-r from-emerald-600/20 to-emerald-500/10 border-white/10" : "bg-gradient-to-r from-emerald-100 to-emerald-50 border-emerald-200"}`}
        >
          <div
          ...
          <div className="relative flex items-start justify-between">
            <div>
              ...
              <span
                className={`inline-block mt-2 px-3 py-1 rounded-full text-xs font-semibold border ${statutStyles[attribution.statut]}`}
              >
                {attribution.statut}
              </span>
            </div>
        ...
        <div className="p-6 space-y-4">
          {[
            ...
            >
              ...
              <span
                className={`font-semibold ${theme.text} text-right max-w-[50%] truncate`}
              >
                {value}
              </span>
            </div>
          ...
          <div
            className={`p-4 rounded-xl border ${isDark ? "bg-white/5 border-white/5" : "bg-gray-50 border-gray-200"}`}
          >
            ...
            {attribution.occupants.length > 0 ? (
              <div className="flex flex-wrap gap-2">
                {attribution.occupants.map((o, i) => (
                  <span
                    key={i}
                    className={`px-3 py-1.5 rounded-lg text-xs font-medium ${isDark ? "bg-emerald-500/20 text-emerald-400" : "bg-emerald-100 text-emerald-700"}`}
                  >
                    {o}
                  </span>
                ))}
...
function ModalForm3D({
  ...
  // Déterminer le type de logement requis 
  const typeRequis =
    alerteBesoin?.typeLogementRequis || attribution?._typeLogementRequis;
  ...
  const [form, setForm] = useState(() => {
    ...
    if (alerteBesoin) {
      const serviceMatch = servicesAvecBesoin.find(
        (s) =>
  ...
  const debutYear = form.date_debut
    ? parseInt(form.date_debut.split("-")[0] || "0", 10)
    ...
    >
      ...
      >
        ...
        <div className="p-6 overflow-y-auto space-y-5">
          ...
          <section
            className={`rounded-2xl border p-5 space-y-4 ${isDark ? "border-white/10 bg-white/5" : "border-gray-200 bg-gray-50"}`}
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label
                  className={`text-xs font-semibold flex items-center gap-1.5 mb-1.5 ${theme.textMuted}`}
                >
                ...
                {!isEdition ? (
                  ...
                  <div
                    className={`w-full px-4 py-3 rounded-xl border ${isDark ? "bg-gray-800/50 border-white/10 text-white/70" : "bg-gray-100 border-gray-200 text-gray-600"} flex items-center gap-2`}
                  >
                    <Building2 className="w-4 h-4 opacity-50" />
                    <span className="font-semibold">
                      {form.departement || "Non défini"}
                    </span>
                  </div>
                )}
                ...
                {!isEdition && servicesAvecBesoin.length === 0 && (
                  <div
                    className={`mt-2 p-3 rounded-xl border ${isDark ? "bg-amber-500/10 border-amber-500/20" : "bg-amber-50 border-amber-200"}`}
                  >
                    <p
                ...
                {!isEdition &&
                  ...
                  (() => {
                    const svcTrouve = servicesAvecBesoin.find(
                      (s) =>
                        `${s.departementNom} — ${s.nom}` === form.departement,
                    );
                    if (!svcTrouve) return null;
                    const nbActifs = svcTrouve.employesActifs?.length || 0;
                    return (
                      <div
                        className={`mt-3 p-3 rounded-xl border ${isDark ? "bg-blue-500/10 border-blue-500/20" : "bg-blue-50 border-blue-200"}`}
                      >
                        <p
                          className={`text-sm font-semibold ${isDark ? "text-blue-300" : "text-blue-700"}`}
                        >
                          <Users2 className="w-4 h-4 inline mr-1" />
                          {nbActifs} employé{nbActifs > 1 ? "s" : ""} actif
                          {nbActifs > 1 ? "s" : ""}
                        </p>
                      </div>
                    );
              ...
              <div>
                ...
                {!isEdition ? (
                  ...
                  >
                    ...
                    <span className="font-semibold">
                      {(() => {
                        const log = logements.find(
                          (l) => l.id === form.logement,
                        );
                        return log
                          ? `${log.id} — ${log.type}`
                          : form.logement || "Non défini";
                      })()}
                    </span>
                  </div>
                )}
                ...
                {!isEdition && logementsSelectionnables.length === 0 && (
                  <div
                    className={`mt-2 p-3 rounded-xl flex items-center gap-2 ${isDark ? "bg-rose-500/15 text-rose-300 border border-rose-500/30" : "bg-rose-100 text-rose-700 border border-rose-300"}`}
                  >
                    <AlertTriangle className="w-4 h-4 flex-shrink-0" />
                    <span className="text-xs font-semibold">
                      Aucun logement disponible conforme au besoin{" "}
                      {typeRequis ? `(${typeRequis} requis)` : ""}
                    </span>
                  </div>
                )}
          ...
          {/* Section: Période */}
          <section
            className={`rounded-2xl border p-5 space-y-4 ${isDark ? "border-emerald-400/20 bg-gradient-to-br from-emerald-500/10 to-transparent" : "border-emerald-300 bg-gradient-to-br from-emerald-50 to-white"}`}
          >
            <div className="flex items-center justify-between">
              <div
                className={`flex items-center gap-2 text-xs font-bold uppercase tracking-widest ${isDark ? "text-emerald-300" : "text-emerald-700"}`}
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                >
                  ...
                  <span
                    className={`ml-1 px-1.5 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider ${isDark ? "bg-emerald-500/20 text-emerald-300" : "bg-emerald-100 text-emerald-700"}`}
                  >
                    Auto
                  </span>
                </label>
                ...
                {dateDebutError ? (
                  <p
                    className={`mt-1.5 text-[11px] font-semibold flex items-center gap-1 ${isDark ? "text-rose-300" : "text-rose-600"}`}
                  >
                  ...
                  <p
                    className={`mt-1.5 text-[10px] flex items-center gap-1 ${theme.textSubtle}`}
                  >
                    <CheckCircle2
              ...
              <div>
                ...
                <input
                  ...
                  onBlur={(e) =>
                    setDateFinError(validateDateISO(e.target.value))
                  }
                ...
                {dateFinError ? (
                  ...
                  <p className={`mt-1.5 text-[10px] ${theme.textLight}`}>
                    Optionnel — laissez vide si indéterminée.
                  </p>
                )}
              </div>
            </div>
          ...
          <section
            className={`rounded-2xl border p-5 space-y-3 ${isDark ? "border-white/10 bg-white/5" : "border-gray-200 bg-gray-50"}`}
          >
            <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-[#C9A84C]">
            ...
            {!isEdition ? (
              <div className="space-y-3">
                <div
                  className={`p-3 rounded-xl flex items-center gap-2 ${isDark ? "bg-blue-500/10 border border-blue-500/20" : "bg-blue-50 border border-blue-200"}`}
                >
                  <CheckCircle2
                  ...
                  <span
                    className={`text-sm font-semibold ${isDark ? "text-blue-300" : "text-blue-700"}`}
                  >
                    Attribution verrouillée — Le statut sera "Occupé" dès la
                    création
                  </span>
                </div>
                ...
                {form.logement &&
                  form.departement &&
                  (() => {
                    const log = logementsSelectionnables.find(
                      (l) => l.id === form.logement,
                    );
                    if (!log) return null;
                    ...
                    const nbFutursOccupants =
                      svcTrouve?.employesActifs?.length || 0;
                    const capaciteMax = CAPACITE_TYPES[log.type] || 1;

                    if (nbFutursOccupants > capaciteMax) {
                      return (
                        <div
                          className={`p-3 rounded-xl border ${isDark ? "bg-rose-500/10 border-rose-500/20" : "bg-rose-50 border-rose-200"}`}
                        >
                          <p
                            className={`text-sm font-bold flex items-center gap-2 ${isDark ? "text-rose-400" : "text-rose-600"}`}
                          >
                            <AlertTriangle className="w-4 h-4 text-yellow-500" />
                            Capacité insuffisante : {nbFutursOccupants}{" "}
                            employés pour {capaciteMax} places max
                          </p>
                        </div>
                      );
                    }
                    ...
                    return (
                      <div
                        className={`p-3 rounded-xl border ${isDark ? "bg-emerald-500/10 border-emerald-500/20" : "bg-emerald-50 border-emerald-200"}`}
                      >
                        <p
                          className={`text-sm font-semibold ${isDark ? "text-emerald-300" : "text-emerald-700"}`}
                        >
                          <Home className="w-4 h-4 inline mr-1" />
                          {nbFutursOccupants} futur
                          {nbFutursOccupants > 1 ? "s" : ""} occupant
                          {nbFutursOccupants > 1 ? "s" : ""} / {capaciteMax}{" "}
                          places max
                        </p>
                      </div>
                    );
              ...
              <div className="space-y-3">
                <div
                  className={`p-3 rounded-xl flex items-center gap-2 ${isDark ? "bg-amber-500/10 border border-amber-500/20" : "bg-amber-50 border border-amber-200"}`}
                >
                  <AlertTriangle
                  ...
                  >
                    Attribution verrouillée — Seule la date de fin peut être
                    modifiée
                  </span>
                </div>
                ...
                {/* Info blocage */}
                <div
                  className={`p-3 rounded-xl border ${isDark ? "bg-white/5 border-white/10" : "bg-gray-100 border-gray-200"}`}
                >
                  <p className={`text-xs ${theme.textLight}`}>
                    <Info className="w-3 h-3 inline mr-1" />
                    Les champs Service, Logement, Statut et Occupants sont
                    verrouillés car l'attribution est en cours.
                  </p>
                </div>
              </div>
            )}
          ...
          {!isEdition && (
            ...
            >
              ...
              {form.occupants && form.occupants.length > 0 ? (
                ...
                <div
                  className={`p-3 rounded-xl border ${isDark ? "bg-blue-500/10 border-blue-500/20" : "bg-blue-50 border-blue-200"}`}
                >
                  <p
          ...
          {error && (
            <div
              className={`p-3 rounded-xl flex items-center gap-2 ${isDark ? "bg-rose-500/15 text-rose-300 border border-rose-500/30" : "bg-rose-100 text-rose-700 border border-rose-300"}`}
            >
              <AlertTriangle className="w-4 h-4 flex-shrink-0" />
        ...
        {/* ======== Footer ======== */}
        <div
          className={`px-6 py-4 border-t flex gap-3 ${isDark ? "border-white/10 bg-black/30" : "border-gray-200 bg-gray-50"}`}
        >
          <button
            onClick={onClose}
            className={`flex-1 py-3 rounded-xl transition font-semibold ${theme.buttonSecondary}`}
          >
            Annuler
          </button>
          <button
            onClick={() => {
              ...
              if (!isEdition) {
                ...
                if (!isLogementConforme) {
                  setError(
                    conformiteError ||
                      "Le logement sélectionné ne satisfait pas la demande du service",
                  );
                  return;
                }
              }
...
function ModalFin3D({ attribution, onClose, onConfirm, isDark }) {
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex items-end sm:items-center justify-center backdrop-blur-md p-0 sm:p-4 animate-in fade-in duration-200 ${isDark ? "bg-black/60" : "bg-gray-900/40"}`}
    >
      <div
        className={`rounded-t-3xl sm:rounded-3xl shadow-2xl w-full sm:max-w-2xl border overflow-hidden animate-in zoom-in-95 duration-300 ${isDark ? "bg-gradient-to-br from-gray-900 to-gray-800 border-white/10" : "bg-white border-gray-200"}`}
      >
        <div className="text-center mb-6">
          <div
            className={`w-16 h-16 rounded-full flex items-center justify-center text-3xl mx-auto mb-4 ${isDark ? "bg-amber-500/20" : "bg-amber-100"}`}
          >
            <AlertTriangle
          ...
          <p className={`text-xs mt-3 ${theme.textLight}`}>
            Le logement sera remis en{" "}
...
export default function Attributions() {
  ...
  const stats = useMemo(() => {
    ...
    const disponibles = attributions.filter(
      (a) => a.statut === "Disponible",
    ).length;
    const maintenance = attributions.filter(
      (a) => a.statut === "Maintenance",
    ).length;
    const totalOccupants = attributions.reduce(
      (sum, a) => sum + a.occupants.length,
      0,
    );
  ...
  const handleSave = async (form) => {
    ...
    if (isEdition) {
      ...
      if (form._alerteId && nouvelleAtt?.id) {
        try {
          await resoudreAlerte(form._alerteId, form.logement);
        } catch (err) {
  ...
  return (
    ...
    >
      ...
      {finData && (
        <ModalFin3D
          attribution={finData}
          onClose={() => setFinData(null)}
          onConfirm={terminerAttribution}
          isDark={isDark}
        />
      )}
      {(formData || isAdding) && (
      ...
      <div className="flex items-start sm:items-center justify-between gap-3">
        ...
        <button
          ...
          className="flex items-center gap-2 px-3 sm:px-5 py-2.5 sm:py-3 bg-gradient-to-r from-[#C9A84C] to-[#a88a3c] text-white rounded-xl font-semibold hover:shadow-lg hover:shadow-amber-500/25 transition-all hover:scale-105 shrink-0"
        >
        ...
        <StatCard3D
          ...
          onClick={() => {
            if (alertesRef.current) {
              alertesRef.current.scrollIntoView({
                behavior: "smooth",
                block: "center",
              });
            }
      ...
      <div
        ...
        style={{
          display:
            alertesBesoins &&
            alertesBesoins.filter((a) => a.statut === "En attente").length > 0
              ? "block"
              : "none",
        }}
      >
        <div
          className={`absolute inset-0 animate-pulse ${isDark ? "bg-gradient-to-r from-amber-500/10 to-transparent" : "bg-gradient-to-r from-amber-200/50 to-transparent"}`}
        />
        <div className="relative flex items-start gap-4">
          <div
            className={`w-12 h-12 rounded-xl flex items-center justify-center animate-bounce ${isDark ? "bg-amber-500/20" : "bg-amber-200"}`}
          >
            <Bell
          ...
          <div className="flex-1">
            {(() => {
              const nbEnAttente = alertesBesoins.filter(
                ...
                .map((alerte) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      departementsBruts.forEach((d) => {
                        ...
                        if (deptMatch) {
                          d.services?.forEach((s) => {
                            if (
                              s.name === alerte.service ||
                              s.nom === alerte.service
                            ) {
                              serviceId = s.id;
                            }
                          });
                      ...
                      // ── MODIFICATION : ouvrir en mode création 
                      setIsAdding(true);
                      setFormData({
                    ...
                    className={`px-3 py-2 rounded-xl text-xs font-semibold transition flex items-center gap-2 max-w-full ${
                      isDark
                  ...
                  >
                    ...
                    <span className="truncate">
                      {alerte.departement} — {alerte.service}
                    </span>
                    <span className="opacity-70 shrink-0">
                      ({alerte.typeLogementRequis})
                    </span>
              ...
              <div>
                ...
                >
                  <div
                    ...
                    style={{
                      width: `${(stats.occupes / (stats.total || 1)) * 100}%`,
                    }}
                  />
                </div>
              </div>
              <div>
                ...
                >
                  <div
                    ...
                    style={{
                      width: `${(stats.disponibles / (stats.total || 1)) * 100}%`,
                    }}
                  />
                </div>
              </div>
              <div>
                ...
                >
                  <div
                    ...
                    style={{
                      width: `${(stats.maintenance / (stats.total || 1)) * 100}%`,
                    }}
                  />
                </div>
              </div>
      ...
      <p className={theme.textLight}>
        {filtered.length} attribution{filtered.length > 1 ? "s" : ""} trouvée
        {filtered.length > 1 ? "s" : ""}
      </p>
      ...
      {viewMode === "table" && (
        <div
          className={`backdrop-blur-sm border rounded-3xl overflow-hidden ${isDark ? "bg-white/5 border-white/10" : "bg-white border-gray-200 shadow-sm"}`}
        >
          <table className="w-full text-sm">
            <thead>
              ...
              >
                {[
                  { label: "Réf", hide: true },
                  { label: "Département", hide: false },
                  { label: "Logement", hide: false },
                  { label: "Période", hide: true },
                  { label: "Occupants", hide: true },
                  { label: "Statut", hide: false },
                  { label: "", hide: false },
                ].map((h) => (
                  <th
                    key={h.label}
                    className={`text-left px-3 sm:px-5 py-4 text-xs font-semibold uppercase tracking-wide ${theme.textLight} ${h.hide ? "hidden sm:table-cell" : ""}`}
                  >
                    {h.label}
                  </th>
                ))}
            ...
            <tbody>
              {filtered.map((a) => (
                ...
                >
                  <td
                    className={`px-3 sm:px-5 py-4 font-mono text-xs ${theme.textLight} hidden sm:table-cell`}
                  >
                    {a.id}
                  </td>
                  <td className={`px-3 sm:px-5 py-4 font-bold ${theme.text}`}>
                    {a.departement}
                  </td>
                  <td className="px-3 sm:px-5 py-4">
                    <span
                      className={`px-2 py-1 rounded-lg text-xs font-bold ${isDark ? "bg-white/10 text-white/80" : "bg-gray-200 text-gray-700"}`}
                    >
                      {a.logement}
                    </span>
                  </td>
                  <td
                    className={`px-3 sm:px-5 py-4 text-xs ${theme.textSubtle} hidden sm:table-cell`}
                  >
                  ...
                  <td className="px-3 sm:px-5 py-4 hidden sm:table-cell">
                    <div className="flex flex-wrap gap-1">
                      {a.occupants.slice(0, 2).map((o, i) => (
                        <span
                          key={i}
                          className={`text-xs px-2 py-0.5 rounded-full ${isDark ? "bg-white/10 text-white/70" : "bg-gray-100 text-gray-600"}`}
                        >
                          {o.split(" ")[0]}
                        </span>
                      ))}
                      {a.occupants.length > 2 && (
                        <span
                          className={`text-xs px-2 py-0.5 rounded-full ${isDark ? "bg-white/5 text-white/50" : "bg-gray-50 text-gray-400"}`}
                        >
                          +{a.occupants.length - 2}
                        </span>
                      )}
            ...
            attributions.map((a) => (
              ...
              >
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  <div
                    className={`w-8 h-8 rounded-lg flex items-center justify-center ${isDark ? "bg-[#C9A84C]/20 text-[#C9A84C]" : "bg-amber-100 text-amber-700"}`}
                  >
                  ...
                  <div className="min-w-0">
                    ...
                    >
                      Logement{" "}
                      <span className="font-mono text-xs">{a.logement}</span>{" "}
                      attribué à {a.departement}
                    </p>
                    <p className={`text-xs ${theme.textLight}`}>
                      Du {a.date_debut}{" "}
                      {a.date_fin
                ...
                <span
                  className={`px-2 py-1 rounded-full text-xs font-semibold border shrink-0 ml-2 ${
                    a.statut === "Occupé"
                      ? isDark
                        ? "bg-emerald-500/20 text-emerald-400 border-emerald-500/30"
                        : "bg-emerald-100 text-emerald-600 border-emerald-300"
