...
const BUDGETS_LOGEMENTS = {
  "LOG-001":5000000,"LOG-002":3000000,"LOG-003":4000000,
  "LOG-004":2000000,"LOG-005":3000000,"LOG-006":3000000,
};
...
// ── Suivi Budget — utilise les vrais départements du contexte ─────────────────
function SuiviBudget({ depenses, logements, tousLesDepartements }) {
  ...
  // Budget par département — calculé dynamiquement
  const depsParDep = {};
  tousLesDepartements.filter(d => d.type === "departement").forEach(d => {
    depsParDep[d.nom] = valides.filter(dep => dep.departement === d.nom || dep.departement.startsWith(d.nom + " —")).reduce((s, dep) => s + dep.montant, 0);
  });
    ...
    <div className="bg-[#F7F5F0] dark:bg-gray-900 rounded-2xl border border-[#E0DDD7] dark:border-gray-700 shadow-sm p-6">
      ...
      <div className="mb-5">
        <div className="flex justify-between text-xs mb-1.5">
          ...
          <span className="text-gray-400">{fmt(total)} / {fmt(BUDGET_GLOBAL)}</span>
        </div>
      ...
      <div className="space-y-2.5 mb-5">
        {logements.map(l => {
          ...
          const budget = BUDGETS_LOGEMENTS[l.id] || 2000000;
      ...
      {/* Par département — dynamique depuis le contexte */}
      <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">Par département</p>
      <div className="space-y-2.5">
        {tousLesDepartements.filter(d => d.type === "departement").map(dep => {
          const montantDep = depsParDep[dep.nom] || 0;
          const budget     = 5000000; // budget par défaut
          const pct        = Math.min((montantDep/budget)*100, 100);
          return (
            <div key={dep.id}>
              <div className="flex justify-between text-xs mb-1">
                <span className="font-semibold text-gray-600 dark:text-gray-300">{dep.nom}</span>
                <span className="text-gray-400">{fmt(montantDep)} / {fmt(budget)}</span>
              </div>
              <div className="h-1.5 rounded-full bg-gray-100 dark:bg-gray-700">
      ...
      <div className="bg-[#F7F5F0] dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-md p-6 border border-[#E0DDD7] dark:border-gray-700">
        <div className="flex justify-between items-start mb-5">
          <div>
            ...
            <h2 className="text-lg font-black text-[#0F2D56] dark:text-white">{depense.description}</h2>
...
// ── Modal Formulaire ──────────────────────────────────────────────────────────
function ModalForm({ depense, logements, tousLesDepartements, onClose, onSave }) {
      ...
      <div className="bg-[#F7F5F0] dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-md p-6 border border-[#E0DDD7] dark:border-gray-700 max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Montant (Ar)</label>
          ...
          <div>
            ...
            <select value={form.statut} onChange={e => setForm({ ...form, statut:e.target.value })}
              className="w-full mt-1 px-3 py-2 rounded-lg border border-[#E0DDD7] dark:border-gray-700 bg-white dark:bg-gray-800 text-[#0F2D56] dark:text-white text-sm focus:outline-none focus:border-[#0F2D56]">
              {["En attente","Validé","Rejeté"].map(s => <option key={s}>{s}</option>)}
            </select>
          </div>
        </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label:"Total général", val:totalGeneral, color:"border-l-[#0F2D56]",  text:"text-[#0F2D56] dark:text-white" },
          { label:"Validées",      val:totalValide,  color:"border-l-emerald-500", text:"text-emerald-500" },
          { label:"En attente",    val:totalAttente, color:"border-l-amber-500",   text:"text-amber-500"   },
          { label:"Rejetées",      val:totalRejete,  color:"border-l-rose-500",    text:"text-rose-500"    },
        ].map(s => (
          <div key={s.label} className={`bg-[#F7F5F0] dark:bg-gray-900 rounded-xl p-4 border border-[#E0DDD7] dark:border-gray-700 border-l-4 ${s.color} shadow-sm`}>
            <p className="text-xs text-gray-400">{s.label}</p>
            <p className={`text-sm font-black mt-1 ${s.text}`}>{fmt(s.val)}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <SuiviBudget depenses={depenses} logements={logements} tousLesDepartements={tousLesDepartements} />
      </div>
      ...
      <p className="text-xs text-gray-400">{filtered.length} dépense{filtered.length > 1 ? "s" : ""}</p>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[#E0DDD7] dark:border-gray-700 bg-[#0F2D56]/5 dark:bg-gray-800">
              {["Réf","Description","Montant","Logement","Fournisseur","Date","Statut",""].map(h => (
          ...
          <tbody>
            {filtered.map(d => (
              <tr key={d.id} className="border-b border-[#E0DDD7]/50 dark:border-gray-800 hover:bg-[#0F2D56]/5 dark:hover:bg-gray-800 transition">
                <td className="px-5 py-3 font-mono text-xs text-gray-400">{d.id}</td>
                <td className="px-5 py-3">
                  <p className="font-semibold text-[#0F2D56] dark:text-white text-xs">{d.description}</p>
                  <p className="text-xs text-gray-400">{d.categorie} — {d.departement}</p>
                </td>
                <td className="px-5 py-3 font-black text-[#C9A84C] text-sm">{fmt(d.montant)}</td>
                <td className="px-5 py-3">
                ...
                <td className="px-5 py-3 text-xs text-gray-500 dark:text-gray-400">{d.fournisseur}</td>
                <td className="px-5 py-3 text-xs text-gray-400">{d.date}</td>
                <td className="px-5 py-3"><StatutDepBadge statut={d.statut}/></td>
                <td className="px-5 py-3">
