...
function ModalDetail({ logement, attributions, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-[#F7F5F0] dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-md p-6 border border-[#E0DDD7] dark:border-gray-700 overflow-y-auto max-h-[90vh]">
...
function ModalForm({ logement, onClose, onSave }) {
  const [form, setForm] = useState(
    logement || { type: "F2", localisation: "", statut: "Disponible", capacite: 2, superficie: 40, nb_occupants_max: 2 }
  );
      ...
      <div className="bg-[#F7F5F0] dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-lg p-6 border border-[#E0DDD7] dark:border-gray-700 overflow-y-auto max-h-[90vh]">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              { label: "Max occupants", key: "nb_occupants_max" },
            ].map(({ label, key }) => (
              <div key={key}>
        ...
        <div className="flex gap-3 mt-6">
          ...
          <button onClick={handleSave} disabled={saving}
            className="flex-1 py-2 bg-[#0F2D56] text-white rounded-xl font-semibold hover:bg-[#1a3f75] transition text-sm flex items-center justify-center gap-2 disabled:opacity-50">
...
export default function Logements() {
  const { logements, loading, ajouterLogement, modifierLogement, supprimerLogement } = useLogements();
  ...
  const handleSave = async (form) => {
    if (formData) await modifierLogement({ ...form, _pk: formData._pk, id: formData.id });
    else await ajouterLogement(form);
  };

  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <Loader2 className="w-8 h-8 animate-spin text-[#0F2D56]" />
        <span className="ml-3 text-gray-500">Chargement des logements...</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      {detail && (
        <ModalDetail logement={detail} attributions={attributions} onClose={() => setDetail(null)} />
      )}
      {(formData || isAdding) && (
        <ModalForm
          logement={formData}
            ...
            return (
              <div key={l.id} className="bg-[#F7F5F0] dark:bg-gray-900 rounded-2xl border border-[#E0DDD7] dark:border-gray-700 shadow-sm p-5 hover:shadow-md transition">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-1 flex items-center gap-1">
                  <MapPin className="w-4 h-4 text-red-500" />{l.localisation}
                </p>
                <p className="text-xs text-gray-400 mb-3 flex items-center gap-1">
                  <PencilRuler className="w-4 h-4 text-yellow-500" />{l.superficie} m²
                </p>
                ...
                <div className="flex gap-2 mt-3">
                  ...
                  <button onClick={() => setFormData(l)}
                    ...
                    <CircleCheck className="w-4 h-4" /> Modifier
                  </button>
