    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-[#F7F5F0] dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-md p-6 border border-[#E0DDD7] dark:border-gray-700">
        ...
        {attLogement.length > 0 && (
          <div className="mt-4">
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Attributions</p>
            {attLogement.map(a => (
              <div key={a.id} className="bg-white dark:bg-gray-800 rounded-xl p-3 mb-2">
                <div className="flex justify-between items-center">
                  <span className="font-semibold text-[#0F2D56] dark:text-white text-sm">{a.departement}</span>
                  <StatutBadge statut={a.statut} />
                </div>
                <p className="text-xs text-gray-400 mt-1">{a.date_debut} → {a.date_fin}</p>
        ...
        <button onClick={onClose} className="mt-4 w-full py-2 bg-[#0F2D56] dark:bg-gray-700 text-white rounded-xl font-semibold hover:bg-[#1a3f75] transition">
          Fermer
        </button>
      </div>
    </div>
...
function ModalForm({ logement, onClose, onSave }) {
  const [form, setForm] = useState(logement || {
    type: "F2", localisation: "", statut: "Disponible",
    capacite: 2, superficie: 40, nb_occupants_max: 2,
  });
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-[#F7F5F0] dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-md p-6 border border-[#E0DDD7] dark:border-gray-700">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Superficie</label>
              <input type="number" value={form.superficie} onChange={e => setForm({ ...form, superficie: Number(e.target.value) })}
                className="w-full mt-1 px-3 py-2 rounded-lg border border-[#E0DDD7] dark:border-gray-700 bg-white dark:bg-gray-800 text-[#0F2D56] dark:text-white text-sm focus:outline-none focus:border-[#0F2D56]" />
            </div>
            <div>
              <label className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Capacité</label>
              <input type="number" value={form.capacite} onChange={e => setForm({ ...form, capacite: Number(e.target.value) })}
                className="w-full mt-1 px-3 py-2 rounded-lg border border-[#E0DDD7] dark:border-gray-700 bg-white dark:bg-gray-800 text-[#0F2D56] dark:text-white text-sm focus:outline-none focus:border-[#0F2D56]" />
            </div>
            <div>
              <label className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Max occupants</label>
              <input type="number" value={form.nb_occupants_max} onChange={e => setForm({ ...form, nb_occupants_max: Number(e.target.value) })}
                className="w-full mt-1 px-3 py-2 rounded-lg border border-[#E0DDD7] dark:border-gray-700 bg-white dark:bg-gray-800 text-[#0F2D56] dark:text-white text-sm focus:outline-none focus:border-[#0F2D56]" />
            </div>
          </div>
          <div>
...
export default function Logements() {
  const { logements, attributions, ajouterLogement, modifierLogement, supprimerLogement } = useApp();
  ...
  const getOccupants = (logId) => {
    const att = attributions.find(a => a.logement === logId && a.statut === "Occupé");
  ...
  return (
    <div className="space-y-5">
      ...
      {detail && <ModalDetail logement={detail} attributions={attributions} onClose={() => setDetail(null)} />}
      {(formData || isAdding) && (
        <ModalForm
          logement={formData}
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="flex gap-1 bg-[#F7F5F0] dark:bg-gray-800 p-1 rounded-xl border border-[#E0DDD7] dark:border-gray-700">
            <button onClick={() => setVue("carte")}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition ${vue === "carte" ? "bg-[#0F2D56] text-white shadow" : "text-gray-500 hover:text-[#0F2D56]"}`}>
              ⊞ Cartes
            </button>
            <button onClick={() => setVue("tableau")}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition ${vue === "tableau" ? "bg-[#0F2D56] text-white shadow" : "text-gray-500 hover:text-[#0F2D56]"}`}>
              ☰ Tableau
            </button>
          </div>
        </div>
      ...
      <p className="text-xs text-gray-400">{filtered.length} logement{filtered.length > 1 ? "s" : ""}</p>
              ...
              <div key={l.id} className="bg-[#F7F5F0] dark:bg-gray-900 rounded-2xl border border-[#E0DDD7] dark:border-gray-700 shadow-sm p-5 hover:shadow-md transition">
                ...
                <div className="border-t border-[#E0DDD7] dark:border-gray-700 pt-3">
                  <div className="flex justify-between text-xs text-gray-400 mb-1.5">
                    <span>Occupants</span>
                    <span>{occupants.length} / {l.nb_occupants_max}</span>
                  </div>
                  <div className="h-1.5 rounded-full bg-[#E0DDD7] dark:bg-gray-700 mb-2">
                    <div className={`h-1.5 rounded-full transition-all ${occupants.length >= l.nb_occupants_max ? "bg-rose-400" : occupants.length > 0 ? "bg-[#C9A84C]" : "bg-gray-300"}`}
                      style={{ width: `${(occupants.length / l.nb_occupants_max) * 100}%` }} />
                  </div>
                  {occupants.length > 0 && (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#E0DDD7] dark:border-gray-700 bg-[#0F2D56]/5 dark:bg-gray-800">
                {["Réf","Type","Localisation","Superficie","Occupants","Statut",""].map(h => (
              ...
              {filtered.map((l) => {
                ...
                return (
                  <tr key={l.id} className="border-b border-[#E0DDD7]/50 dark:border-gray-800 hover:bg-[#0F2D56]/5 dark:hover:bg-gray-800 transition">
                    ...
                    <td className="px-5 py-3 text-xs text-gray-500 dark:text-gray-400">{l.localisation}</td>
                    <td className="px-5 py-3 text-xs text-gray-500 dark:text-gray-400">{l.superficie} m²</td>
                    <td className="px-5 py-3 text-xs">
                      <span className={`font-semibold ${occupants.length >= l.nb_occupants_max ? "text-rose-500" : "text-gray-600 dark:text-gray-400"}`}>
