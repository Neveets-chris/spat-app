    ...
    >
      <div className="relative w-32 h-32">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-2xl font-bold text-[#0F2D56] dark:text-white">
            {value}%
          </span>
        </div>
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between mb-4">
        <div
          className={`p-3 rounded-xl bg-gradient-to-br ${couleur} shadow-lg`}
        >
        ...
        <div className="flex flex-col items-end gap-2">
          {trend && (
            <div
              className={`flex items-center gap-1 text-xs font-semibold px-2 py-1 rounded-full ${trend === "up" ? "bg-emerald-100 text-emerald-600 dark:bg-emerald-900/30" : "bg-rose-100 text-rose-600 dark:bg-rose-900/30"}`}
            >
              {trend === "up" ? (
...
export default function Dashboard() {
  ...
  const tauxOccupation =
    stats.logTotal > 0
      ? Math.round((stats.logOccupes / stats.logTotal) * 100)
      : 0;
  const tauxMaintenance =
    stats.logTotal > 0
      ? Math.round((stats.logMaintenance / stats.logTotal) * 100)
      : 0;
  ...
  const depensesTrend =
    avantDernierMois && parMois[avantDernierMois] > 0
      ? Math.round(
          ((parMois[dernierMois] - parMois[avantDernierMois]) /
            parMois[avantDernierMois]) *
            100,
        )
      : 0;
...
>>>>>>> 670e4d0787ab6a11494c74c41e809c5f1aa4d553
  ...
  const greedIndex = Math.max(
    0,
    Math.min(100, Math.round(tauxOccupation - tauxMaintenance * 2)),
  );
  ...
  const STATS = [
    {
      ...
      trendValue: Math.abs(
        Math.round(
          ((stats.logDisponibles - stats.logTotal * 0.3) /
            (stats.logTotal * 0.3)) *
            100,
        ),
      ),
  ...
  const depMois = Object.entries(parMois)
    .slice(-6)
        ...
        >
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-4">
              ...
              <Sparkline
                data={
                  sparkDataDepenses.length > 1
                    ? sparkDataDepenses
                    : [1, 1.2, 1.1, 1.3, 1.5, 1.8]
                }
                color="#C9A84C"
              />
            </div>
            ...
            <div className="flex items-center gap-2 text-xs">
              <span
                className={`flex items-center gap-1 px-2 py-0.5 rounded-full ${depensesTrend >= 0 ? "bg-emerald-500/20 text-emerald-300" : "bg-rose-500/20 text-rose-300"}`}
              >
                {depensesTrend >= 0 ? (
                  <TrendingUp className="w-3 h-3" />
                ) : (
              ...
              <div className="flex-1">
                ...
                <div className="h-1.5 rounded-full bg-gray-100 dark:bg-gray-800">
                  <div
                    ...
                    style={{
                      width: `${Math.min((alertesStock.length / materiaux.length) * 100, 100)}%`,
                    }}
                  />
                </div>
              </div>
        ...
        >
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="text-right">
              ...
              <p className="text-lg font-bold text-[#0F2D56] dark:text-white">
                {materiaux.length}
              </p>
            </div>
          </div>
            ...
            {materiaux.map((m, i) => {
              ...
              return (
                <div
                  key={m.nom}
                  style={{
                    animation: `slideIn 0.4s ease-out ${0.9 + i * 0.05}s both`,
                  }}
                >
                  <div className="flex justify-between text-xs mb-1.5">
                    <span
                      className={`font-semibold flex items-center gap-1 ${alerte ? "text-rose-500" : "text-gray-700 dark:text-gray-300"}`}
                    >
      ...
      >
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              {depEnAttente.length} dépense{depEnAttente.length > 1 ? "s" : ""}{" "}
              à valider
            </p>
          </div>
        ...
        {depEnAttente.length > 0 ? (
              ...
              <thead>
                <tr className="border-b border-gray-100 dark:border-gray-800">
                  {[
                    ...
                    <th
                      key={h}
                      className="text-left py-3 text-xs text-gray-400 font-semibold uppercase tracking-wide"
                    >
                      {h}
                    </th>
                  ))}
              ...
              <tbody>
                {depEnAttente.map((d, i) => (
                  <tr
                    ...
                    style={{
                      animation: `slideIn 0.4s ease-out ${1 + i * 0.05}s both`,
                    }}
                  >
                    <td className="py-3 font-mono text-xs text-gray-400">
                      #{d.id}
                    </td>
                    <td className="py-3 font-semibold text-gray-800 dark:text-gray-200 text-xs group-hover:text-[#0F2D56] dark:group-hover:text-[#C9A84C] transition-colors">
                      {d.description}
                    </td>
                    <td className="py-3 font-black text-[#C9A84C] text-xs">
                      {fmt(d.montant)}
                    </td>
                    <td className="py-3">
          ...
          <div className="text-center py-8 text-gray-400">
            ...
            <p className="text-sm"><CheckCheck className="w-4 h-4 text-green-500"/> Toutes les dépenses sont validées</p>
          </div>
        )}
      </div>
    </div>
