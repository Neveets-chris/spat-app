...
export default function Logi() {
  ...
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      content:
        "Bonjour ! Je suis **Logi**, votre assistant logistique. Dites-moi par exemple : *«entrée de 50 parpaings»* ou *«quel est le stock actuel ?»*",
    },
  ]);
  ...
  const callClaude = async (userMessage) => {
    ...
    const raw =
      data.content ||
      '{"texte":"Erreur de communication.","action":null,"confirmation":false}';

    let parsed;
    try {
      parsed = JSON.parse(raw);
    } catch {
      parsed = { texte: raw, action: null, confirmation: false };
    }
  ...
  const executerAction = async (action) => {
    ...
    if (action.type === "stock") {
      try {
        ...
        const lines = data
          .map(
            (m) =>
              `• **${m.nom}** : ${m.stock} ${m.unite}${m.en_alerte ? " ⚠️" : ""}`,
          )
          .join("\n");
        return `📦 **Stock actuel :**\n${lines}`;
      } catch {
    ...
    if (action.type === "alertes") {
      try {
        ...
        const data = await res.json();
        if (!data.length)
          return "✅ Aucune alerte — tout est bien approvisionné.";
        return (
          "⚠️ **Stocks bas :**\n" +
          data
            .map((m) => `• **${m.nom}** : ${m.stock}/${m.seuil} ${m.unite}`)
            .join("\n")
        );
      } catch {
        return "❌ Impossible de joindre le serveur.";
      ...
      try {
        const res = await fetch(`${API_BASE}/api/logi/mouvement/`, {
          ...
          body: JSON.stringify({
            ...
            fournisseur:
              d.typeOp === "Entrée" ? d.fournisseur || "Non précisé" : "",
          }),
        });
        const data = await res.json();
        ...
        ajouterMouvement({
          ...
          logement: d.logement || logements[0]?.id || "LOG-001",
          fournisseur:
            d.typeOp === "Entrée" ? d.fournisseur || "Non précisé" : null,
        });
  ...
  const startMic = () => {
    ...
    if (!SR) {
      setMicError("Utilisez Chrome ou Edge.");
      return;
    }
    setMicError(null);
    const recog = new SR();
  ...
  const renderMsg = (msg, i) => {
    ...
    if (isSystem)
      return (
        <div
          key={i}
          className="my-2 px-3 py-2 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-sm"
        >
    ...
    return (
      ...
      >
        {!isUser && (
          <div className="w-7 h-7 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white text-xs font-bold mr-2 mt-1 flex-shrink-0">
            L
          </div>
        )}
        <div
          className={`max-w-[80%] px-3 py-2 rounded-2xl text-sm leading-relaxed ${isUser ? "bg-blue-600 text-white rounded-tr-sm" : "bg-white text-gray-800 shadow-sm border border-gray-100 rounded-tl-sm"}`}
        >
          {renderText(msg.content)}
        </div>
      </div>
  ...
  return (
    <>
      <button
        onClick={() => setOpen((v) => !v)}
        className="fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 text-white shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-200 flex items-center justify-center"
      >
      ...
      {open && (
        <div
          className="fixed bottom-24 right-6 z-50 w-80 sm:w-96 h-[520px] bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-gray-200"
          style={{ animation: "logiSlideIn 0.2s ease-out" }}
        >
              ...
              <div className="flex gap-2 mt-2 justify-end">
                <button
                  onClick={() => {
                    ...
                    setMessages((p) => [
                      ...p,
                      { role: "system", content: "❌ Action annulée." },
                    ]);
                  }}
          ...
          {micError && (
            <div className="px-4 py-1 text-xs text-red-500 bg-red-50 text-center">
              {micError}
            </div>
          )}
          ...
          <div className="px-3 py-3 border-t border-gray-100 bg-white flex items-center gap-2">
            ...
            <button
              ...
              className="p-2 rounded-xl bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
            >
              <IconSend />
            </button>
          </div>
