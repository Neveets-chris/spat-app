...
function NavItem({ item, isActive, isOpen, onClick }) {
  ...
  return (
    ...
    >
      ...
      {isActive && (
        <div
          className={`absolute inset-0 bg-gradient-to-r ${item.color} opacity-90`}
        />
      )}
      ...
      {/* Icon container */}
      <span
        className={`relative z-10 shrink-0 p-2 rounded-lg transition-all duration-300 ${
      ...
      <span
        className={`relative z-10 truncate transition-all duration-300 ${
          isOpen
            ...
            : "opacity-0 -translate-x-4 w-0 md:hidden"
        }`}
      >
        {item.label}
      </span>
...
<div className="flex items-center gap-3 px-4 py-5 border-b border-[#1a3f75]/30 dark:border-gray-800/50 ">
  <div className="w-10 h-10 rounded-xl overflow-hidden shrink-0 ring-2 ring-white/5 shadow-lg  shadow-[#C9A84C]/20  dark:bg-white bg-[#3B82F6] ">
    <img 
  ...
  <div className={`overflow-hidden transition-all duration-500 ${open ? "opacity-100 w-auto" : "opacity-0 w-0 md:hidden"}`}>
    <p className="font-black text-white text-sm whitespace-nowrap tracking-wide">SpatLoge</p>
      ...
      Gestion de Logements
        ...
        <nav className="flex-1 py-6 px-2 space-y-1">
          {NAV_ITEMS.map((item, index) => (
            <div
              ...
              className={
                open ? "animate-in slide-in-from-left-4 duration-300" : ""
              }
            >
              <NavItem
                item={item}
        ...
        <div className="p-3 relative" ref={profilRef}>
          ...
          >
            ...
            <div
              className={`overflow-hidden transition-all duration-300 flex-1 text-left ${open ? "opacity-100 w-auto" : "opacity-0 w-0 md:hidden"}`}
            >
            ...
            {open && (
              <ChevronRight
                className={`w-4 h-4 text-white/40 transition-transform duration-300 ${profilOpen ? "rotate-90" : ""}`}
              />
            )}
          </button>
        </div>
