...
export default function Header({ titre, setSidebarOpen, sidebarOpen, nbNotifs, darkMode, setDarkMode }) {
  ...
  const notifications = [
    // Dépenses en attente
    ...depenses.filter(d => d.statut === "En attente").map(d => ({
      id:      `dep-${d.id}`,
      type:    "warning",
      icon:    "💰",
    ...
    // Stock bas
    ...materiaux.filter(m => m.stock <= m.seuil).map(m => ({
      id:      `mat-${m.id}`,
      type:    "danger",
      icon:    "⚠️",
      titre:   "Stock bas",
    ...
    ...logements.filter(l => l.statut === "Maintenance").map(l => ({
      id:      `log-${l.id}`,
      type:    "info",
      icon:    "🔧",
    ...
    ...attributions.filter(a => a.statut === "Occupé").slice(0, 2).map(a => ({
      id:      `att-${a.id}`,
      type:    "success",
      icon:    "🏠",
    ...
    <header className="bg-[#0F2D56] dark:bg-gray-900 border-b border-[#1a3f75] dark:border-gray-800 px-4 md:px-6 py-3 flex items-center justify-between shrink-0">
      ...
      <div className="flex items-center gap-2 md:gap-3"> </div>
        ...
        <div className="relative">
          <button className="w-8 h-8 rounded-lg bg-white/10 hover:bg-white/20 flex items-center justify-center text-sm transition"> </button>
